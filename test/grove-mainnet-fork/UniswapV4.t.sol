...
contract MainnetControllerUniswapV4Tests is ForkTestBase {
    ...
    function test_mintPositionUniswapV4_revertsWhenMaxSlippageNotSet() external {
        vm.startPrank(GROVE_PROXY);
        mainnetController.setUniswapV4AddLiquidityLowerTickBound(_POOL_ID, -60);
        mainnetController.setUniswapV4AddLiquidityUpperTickBound(_POOL_ID, 60);
        vm.stopPrank();
    ...
    function test_mintPositionUniswapV4_revertsWhenAmount0MaxTooHigh() external {
        ...
        _setAddLiquidityTickBounds(_POOL_ID, -60, 60);
    ...
    /**********************************************************************************************/
    /*** setUniswapV4AddLiquidityTickBounds Tests                                               ***/
    /**********************************************************************************************/
    function test_setUniswapV4AddLiquidityTickBounds_revertsForNonAdmin() external {
        vm.expectRevert(
            abi.encodeWithSelector(
                IAccessControl.AccessControlUnauthorizedAccount.selector,
                _unauthorized,
        ...
        vm.prank(_unauthorized);
        mainnetController.setUniswapV4AddLiquidityLowerTickBound(bytes32(0), 0);
        ...
        vm.prank(_unauthorized);
        mainnetController.setUniswapV4AddLiquidityUpperTickBound(bytes32(0), 0);
    }

    function test_setUniswapV4AddLiquidityTickBounds_revertsWhenInvalidTicks() external {
        vm.startPrank(GROVE_PROXY);
        vm.expectRevert("MainnetController/lower-tick-out-of-bounds");
        mainnetController.setUniswapV4AddLiquidityLowerTickBound(bytes32(0), 1);

        mainnetController.setUniswapV4AddLiquidityLowerTickBound(bytes32(0), -10);

        vm.expectRevert("MainnetController/upper-tick-out-of-bounds");
        mainnetController.setUniswapV4AddLiquidityUpperTickBound(bytes32(0), -10);
        vm.stopPrank();
    }

    function test_setUniswapV4AddLiquidityTickBounds() external {
        vm.startPrank(GROVE_PROXY);
        mainnetController.setUniswapV4AddLiquidityLowerTickBound(_POOL_ID, -60);

        UniswapV4Lib.UniswapV4PoolParams memory expectedParams;
        expectedParams.addLiquidityTickBounds = UniswapV4Lib.Tick({ lower: -60, upper: 60 });

        vm.expectEmit(address(mainnetController));
        emit MainnetController.UniswapV4PoolParamsUpdated(_POOL_ID, expectedParams);

        mainnetController.setUniswapV4AddLiquidityUpperTickBound(_POOL_ID, 60);
        vm.stopPrank();

        UniswapV4Lib.Tick memory limits = mainnetController.uniswapV4PoolParams(_POOL_ID);

        assertEq(limits.lower, -60);
        assertEq(limits.upper, 60);
    }
...
/**********************************************************************************************/
    ...
    function _setAddLiquidityTickBounds(bytes32 poolId, int24 lower, int24 upper) internal {
        mainnetController.setUniswapV4AddLiquidityLowerTickBound(poolId, lower);
        mainnetController.setUniswapV4AddLiquidityUpperTickBound(poolId, upper);
    }
    ...
    function _setupForLiquidityIncrease() internal returns (IncreasePositionResult memory minted) {
        ...
        _setAddLiquidityTickBounds(_POOL_ID, 0, 1);
        vm.stopPrank();
    }
